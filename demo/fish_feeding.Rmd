...
longer <- expand.grid(time = time, axis = axis, lat = lat, lon = lon,  
...
array <- array(
  ...
  dim = c(length(time), length(axis), length(lat), length(lon))
)
```


```{r coerce_ices_into_array, eval = F}
# First add an unique identifier by the combination of: 
# decimaLongitude, decimalLatitude, eventDate and AphiaID
...
dataset$ices <- as.numeric(dataset$ices)
...
var.def.nc(nc, varname = "axis_score", vartype = "NC_DOUBLE", dimensions = c("time", "axis", "lat", "lon"))
...
var.def.nc(nc, varname = "test", vartype = "NC_DOUBLE", dimensions = c("lon", "lat"))
# Add attributes
...
var.put.nc(nc, variable = "test", data = array_ices)
...
attributes <- list(
  ...
  citation = "Murray et al (2025). Integrated data product created under EMODNet Biology: Biomass-Weighted Fish Functional Feeding Traits for the Northeast Atlantic (1997-2020).",
